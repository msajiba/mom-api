...
const handler = async (req, res) => {
  ...
  if (contentType && contentType.indexOf("multipart/form-data") !== -1) {
    ...
    const fileProcessingPromises = fileKeys.map(async (fileKey) => {
      ...
      const id = uuidv4();
      const filePath = path.join(uploadDir, `${id}.docx`);

      await fsPromises.copyFile(file.filepath, filePath);

      // Compress PDF using Ghostscript
      const pdfToWordFilePath = path.join(uploadDir, `${id}_pdfToWord.docx`);
      const command = `gs -sDEVICE=docxwrite -o ${pdfToWordFilePath} ${filePath}`;
